...
!     in the future):xy
        ...
        do j = 1, Nar
           ...
           write(*,*)"Region Layers, ",j,Nregion(j)
         Nrad = Nrad + Nregion(j)
        end do
        write(*,*) 'Nrad = ', Nrad
...
!     Split and fill with coordinates:
        ...
        rtotal = 0.0
        do j = 1, Nrad
         Radii(j) = rmax*(1.0 - (real(j)-1.0)/real(Nrad))
!           Radii(j) = rmax*cos((real(j)-0.5)/real(Nrad)*0.5*pi)
           if(j .gt. 1)then
              rtotal = rtotal + Radii(j-1) - Radii(j)
           endif
        end do
        write(*,*)"Total radius ",rtotal,rmax
